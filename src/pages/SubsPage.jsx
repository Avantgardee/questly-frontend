...
import { fetchGetSubs, setSearchQuery } from '../redux/slices/subs';
import { Button, Grid, Card, CardContent, CardActions, Avatar, Typography, Box, Skeleton, CircularProgress } from '@mui/material';
...
const SubscriptionsOrSubscribersPage = () => {
    ...
    useEffect(() => {
        ...
        dispatch(fetchGetSubs({ id, group, page: 1, limit: 10, search: '', append: false }));
    }, [id, group, dispatch]);

    const loadMoreUsers = useCallback(() => {
        if (!isLoadingMore && hasMore && id && group) {
            const nextPage = currentPageRef.current + 1;
            dispatch(fetchGetSubs({ 
                id, 
                group, 
                page: nextPage, 
                limit: 10, 
    ...
    }, [isLoadingMore, hasMore, id, group, searchQuery, dispatch]);
    ...
    const handleSubscription = async (userId, isSubscribed) => {
        try {
            ...
            if (response.status === 200) {
                ...
                currentPageRef.current = 1;
                dispatch(fetchGetSubs({ id, group, page: 1, limit: 10, search: searchQuery, append: false }));
            } else {
